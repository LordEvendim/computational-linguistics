...
class Head(nn.Module):
    ...
    def forward(self, x):
        ...
        # reshape: (batch, seqlen, 1, headdim)
        q = q.unsqueeze(2)  # (B, T, 1, head_size)
        k = k.unsqueeze(2)  # (B, T, 1, head_size)
        v = v.unsqueeze(2)  # (B, T, 1, head_size)
        ...
        out = flash_attn_func(
            ...
            softmax_scale=None,  # 1 / sqrt(d) by default
            causal=True,
        )

        # (B, T, 1, head_size) -> (B, T, head_size)
        out = out.squeeze(2)
