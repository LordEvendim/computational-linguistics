...
def compute_perplexity(model, text, encoder, device):
    ...
    with torch.no_grad():
        for i in range(0, len(tokens) - 1, block_size):
            ...
            x = torch.tensor(
                tokens[i:end_idx], dtype=torch.long, device=device
            ).unsqueeze(0)
            y = torch.tensor(
                tokens[i + 1 : end_idx + 1], dtype=torch.long, device=device
            ).unsqueeze(0)
...
def compute_inference_time(model, text, encoder, device, num_runs=10):
    ...
    y = torch.tensor(
        tokens[1 : max_len + 1], dtype=torch.long, device=device
    ).unsqueeze(0)
...
def compute_oov_statistics(tokenizer, text):
    ...
    return {
        ...
        "unique_oov_percentage": (
            (unique_oov / len(unique_words) * 100) if len(unique_words) > 0 else 0
        ),
...
def analyze_sample_text(text, tokenizers):
    ...
    for name, tokenizer in tokenizers.items():
        ...
        for word in words:
            ...
            if len(word_tokens) == 1:
                ...
                if word_decoded.lower() == word.lower() or (
                    isinstance(tokenizer, WhitespaceTokenizer)
                    and word in tokenizer.word_to_idx
                ):
                    words_direct += 1
        ...
        print(
            f"-> Words encoded directly:   {words_direct} / {num_words} ({words_direct_pct}%)"
        )
    ...
    for name, result in results.items():
        print(
            f"{name:<20} {result['num_tokens']:<10} {result['tokens_per_word']:<12.2f} {result['words_direct']}/{result['num_words']} ({result['words_direct_pct']:.1f}%)"
        )
...
def perform_qualitative_analysis(tokenizers, corpus):
    ...
    for tokenizer_name in tokenizers.keys():
        avg_tokens_per_word = sum(
            all_results[sample][tokenizer_name]["tokens_per_word"]
            for sample in all_results
        ) / len(all_results)
        avg_direct_pct = sum(
            all_results[sample][tokenizer_name]["words_direct_pct"]
            for sample in all_results
        ) / len(all_results)
...
def main():
    ...
    models = {
        "p50k_base": "checkpoints/checkpoint_transformer_p50k_base_4999.pt",
        "whitespace": "checkpoints/checkpoint_transformer_whitespace_4999.pt",
        "sentencepiece": "checkpoints/checkpoint_transformer_sentencepiece_4999.pt",
    }
    ...
    for model_name, model_path in models.items():
        ...
        # Perplexity metrics
        word_ppl, char_ppl, num_tokens = compute_perplexity(
            model, test_text, encoder, device
        )
        ...
        print(
            f"-> Tokens per word:            {num_tokens / len(split(test_text)):.2f}"
        )
        ...
        print(
            f"-> Words in dictionary:      {efficiency['words_in_dict']} / {efficiency['total_words']} ({efficiency['words_in_dict_pct']:.2f}%)"
        )

        # Inference time
        avg_time, model_throughput = compute_inference_time(
            model, test_text, encoder, device
        )
        ...
        if model_name == "whitespace":
            ...
            print(
                f"-> OOV words:                {oov_stats['oov_words']} / {oov_stats['total_words']} ({oov_stats['oov_percentage']:.2f}%)"
            )
            print(
                f"-> Unique OOV words:         {oov_stats['unique_oov_words']} / {oov_stats['unique_total_words']} ({oov_stats['unique_oov_percentage']:.2f}%)"
            )
